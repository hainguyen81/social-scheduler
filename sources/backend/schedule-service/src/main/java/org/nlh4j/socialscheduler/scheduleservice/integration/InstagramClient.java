package org.nlh4j.socialscheduler.scheduleservice.integration;
    ...
    public Object publish(ScheduleEntity schedule) {
        ...
        try {
            ...
            return restClient.post()
                    .uri("/{ig-user-id}/media", schedule.getUserId().toString())
                    .header("Authorization", "Bearer " + accessToken)
                    .body(Map.of("image_url", schedule.getContent()))
                    .retrieve()
                    .onStatus(HttpStatusCode::is4xxClientError, (request, response) -> {
                        throw new SocialPlatformException(Platform.INSTAGRAM, "CLIENT_ERROR", HttpStatus.resolve(response.getStatusCode().value()), false);
                    })
                    .onStatus(HttpStatusCode::is5xxServerError, (request, response) -> {
                        throw new SocialPlatformException(Platform.INSTAGRAM, "SERVER_ERROR", HttpStatus.resolve(response.getStatusCode().value()), true);
                    })
                    .body(Object.class);

        } catch (HttpClientErrorException | HttpServerErrorException e) {
            log.error("Instagram API error: {}", e.getMessage());
            throw new SocialPlatformException(Platform.INSTAGRAM, "API_ERROR", HttpStatus.resolve(e.getStatusCode().value()), true);
        } catch (ResourceAccessException e) {
            log.error("Instagram API timeout or network error: {}", e.getMessage());
            throw new SocialPlatformException(Platform.INSTAGRAM, "NETWORK_TIMEOUT", null, true);
        } finally {
            MDC.clear();
        }
    }
