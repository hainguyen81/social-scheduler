...
public class TikTokClient {
    ...
    public Object publish(ScheduleEntity schedule) {
        ...
        try {
            ...
            throw new SocialPlatformException(Platform.TIKTOK, "HTTP_ERROR", HttpStatus.resolve(e.getStatusCode().value()), true);
        } catch (ResourceAccessException e) {
            log.error("TikTok API connection timeout or network failure", e);
            throw new SocialPlatformException(Platform.TIKTOK, "NETWORK_TIMEOUT", null, true);
        } finally {
            MDC.clear();
        }
    }
