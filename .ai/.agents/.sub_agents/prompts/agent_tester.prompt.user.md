...
* **Verification Scope:** {{ source_component }}
...
{% if not existing_source_component %}
...
Verify that any Java file generated strictly begins with the required `org.nlh4j.{{ project_name | lower | replace(" ", "") | replace("_", "") | replace("-", "") }}` structure. Ensure that you read the exact Tag IDs from the `{{ targeted_tags }}` variable, and permanently burn those codes into the Javadoc metadata blocks (for Java) or the test case description strings (for TypeScript/JavaScript). Emit the final compiler-ready production source code inside a single valid markdown code block now.
