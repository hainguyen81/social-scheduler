...
- **Java Package Enforcement:** For any Java backend file, the very first line of code MUST declare or align with the corporate enterprise package prefix layout: `package org.nlh4j.{{ project_name | lower | replace(" ", "") | replace("_", "") | replace("-", "") }}.[sub_package];` (where `{{ project_name }}` is dynamically substituted with the lower-case token of the target project, e.g., `membershiphub` or `cashflow`).
...
- **THE REFACTORING INVARIANT LAW (ANTI-REGRESSION GATE):** If an existing version of the file is provided within the `<EXISTING_SOURCE_CODE>` block, you ARE PERMANENTLY BANNED from deleting, rewriting, or altering any structural legacy business methods, pre-existing validation annotations, or historical security filters that are not explicitly targeted by the active sub-task. You MUST operate as an incremental refactoring engine. You MUST weave the new feature seamlessly into the current code architecture, maintaining 100% backward compatibility, and protecting existing entity relationships or API contract behaviors from corruption.
{% endif %}
